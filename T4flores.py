...
'''
- Dataset: colección de datos estructurados (matriz de características + etiquetas).
- Features: variables independientes que se usan para predecir (columnas del dataset).
- Labels: variable dependiente o resultado esperado (categoría o valor a predecir).
- train_test_split: función que divide el dataset en datos de entrenamiento y prueba.
- LogisticRegression: modelo estadístico que se usa para clasificación binaria o multiclase.
- fit(): entrena el modelo con los datos de entrenamiento.
- predict(): realiza predicciones con el modelo ya entrenado.
- accuracy_score: mide qué porcentaje de las predicciones son correctas.
'''